let
    ...
    let
        Source = try Json.Document(Web.Contents("https://api.wonde.com", [
            RelativePath = "v1.0/schools/A121320526/students/?include=groups,extended_details&page=" & Number.ToText(page),
        ...
        ResultTable = if RawData = null or List.Count(RawData) = 0 then EmptyTable else
            let
                ...
                Expanded = Table.ExpandRecordColumn(Records, "Record", {
                    "id", "upi", "surname", "forename", "gender", "extended_details", "groups"
                }),
                ...
                Groups = Table.ExpandRecordColumn(ExtData, "groups", {"data"}, {"groups_data"}),
                ExpandedGroups = Table.ExpandListColumn(Groups, "groups_data"),
                GroupDetails = Table.ExpandRecordColumn(ExpandedGroups, "groups_data", {"code", "id", "type"}, {"YearCode", "GroupID", "GroupType"}),
                YearGroups = Table.SelectRows(GroupDetails, each [GroupType] = "YEAR"),
                WithName = Table.AddColumn(YearGroups, "Student Name", each [surname] & ", " & [forename]),
                Renamed = Table.RenameColumns(WithName, {
                    ...
                    {"premium_pupil_indicator", "Pupil Premium"}, {"YearCode", "Year"},
                    {"ethnicity", "Ethnicity"}
                }),
    ...
    // Create list of 100 page numbers
    ...
    AddStatus = Table.AddColumn(AddSchoolName, "Status", each "Active", type text)
in
    AddStatus
